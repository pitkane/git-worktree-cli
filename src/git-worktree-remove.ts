...
async function findWorktreesFromExisting(
  rootPath: string
): Promise<Worktree[]> {
  ...
  try {
    const output =
      await $`cd ${foundWorktree} && git worktree list --porcelain`.text();
...
async function gwtremove(branchName?: string) {
  try {
    ...
    if (!branchName) {
      ...
      if (!targetWorktree) {
        ...
        for (const worktree of worktrees) {
          const cleanBranch = worktree.branch
    ...
    const cleanBranch = targetWorktree.branch
      ? targetWorktree.branch.replace("refs/heads/", "")
      : targetWorktree.HEAD.slice(0, 7);
